...
const query = urlParams.get('query')== null ? '':urlParams.get('query');

async function fetchJobs() {
    const url = 'http://18.141.180.165:80/search'; // URL API
    ...
    try {
        const response = await fetch(url, {
            ...
            body: JSON.stringify({ query: query }) // Dữ liệu gửi đi
        });
        ...
        const jobsResult = result.results;
                ...
                `<div class="col-xl-4 col-md-6">
                    <div class="ec-job-item d-flex align-items-center justify-content-between">
                        ...
                        <div class="ec-job-item-content ms-3">
                            <a class="ec-job-name text-dark text-decoration-none" href="https://deadinside.pages.dev/public/details?id=${job.title}">${job.title}</a>
        ...
        if (token) {
            ...
            document.getElementById('accountBT1').href = "https://deadinside.pages.dev/public/profile";
            document.getElementById('accountBT2').href = "https://deadinside.pages.dev/public/profile";
